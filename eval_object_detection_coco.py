...
import torch
from art.estimators import object_tracking
...
parser.add_argument('--skip_benign', action='store_true', default=True)
...
parser.add_argument('--union', action='store_true', default=True)
...
#print("data_dir path: ", os.path.join(data_dir, "*pth"))
#data_files = sorted(glob.glob(os.path.join(data_dir, "*pth")))

# data files path = adv_data\coco_random_patch_100\data
data_dir = "adv_data/coco_random_patch_100/data/"
...
if args.skip_benign:
    ...
    for batch_idx, data_file in enumerate(tqdm(data_files[0:args.n_batches], desc="Benign")):
        ...
        x = data['x']
        y = data['y']
        # check if x and y are null or not
