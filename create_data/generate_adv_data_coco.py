...
if args.random:
    ...
    save_dir = f'full_dataset/coco_topleft_patch_{args.patch_size}'
...
# setup dataset
dataset = get_coco("./lisa/", image_set="train", transforms=presets.DetectionPresetEval())
...
for i, data in enumerate(pbar):
    ...
    x_adv_bgr = cv2.cvtColor((x_adv[0] * 255).astype(np.uint8), cv2.COLOR_RGB2BGR)

    # save adv img
    img_fn = os.path.join(img_dir, f'{starting_num:06d}.png')
    cv2.imwrite(img_fn, x_adv_bgr)
